...
- `petergyang/no-ai-slop` (MIT) — checked against this skill on 2026-09-02: 21 of its 23 named patterns were already covered. Two were not, and are now in `ai-patterns.md`: **colon reveals** and **fake-profound kickers**. Its `eval.md` is a numbered pass/fail list the model answers after editing, and that form is the basis of `references/self-check.md` — the one place its design beat ours, because a numbered list is harder to narrate past than a prose audit. Its **portability test** is adopted as core rule 4. **Its em-dash rule is deliberately not carried over**: a flat prohibition, where `ai-patterns.md` holds an author-relative rule measured against a real corpus. It ships no code at all, so every judgement in it is the model asserting.

**The non-native-writer boundary in core rule 3, and `--against`.** Measured 2026-08-15 on offbeat-music.com: 12 pieces, 12,425 words of authorial prose (quoted speech split out), 2019–2024, by a confirmed non-native English writer. Her contraction rate is a median of **2.9 per 1000 words**, band 0.0–5.6, with one published piece at exactly **0.0** — so Layer 3's "restore contractions the draft expanded" had nothing to restore and moved her away from herself, with nothing in the report saying so. Constructions a native-idiom pass would "fix", all hers and all published: *"Calgary-born, Vancouver located musician"*, *"her voice … but need in no way hide"*, *"It is your livingroom dolling up as a venue"*.

**n=1 author.** That is strong enough to change how the skill behaves and not strong enough to state as a norm about non-native writers in general. Median 2.9 is not a target, and nothing in the skill treats it as one.

The finding also exposes an asymmetry worth naming: the Liang et al. citation above guards the **detection** question, and until 2026-09-03 nothing guarded the **rewrite** question. A skill can refuse to score someone as AI and still edit them toward a register that is not theirs.

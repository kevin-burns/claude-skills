...
| [excalidraw-diagram](./excalidraw-diagram) | Generate Excalidraw diagrams that argue visually, with a fully-offline render→view→fix loop (vendored engine — no CDN, works in sandboxes) and optional cloud-icon (AWS/Azure/GCP) ingestion | [Excalidraw](https://github.com/excalidraw/excalidraw) (MIT, vendored); design forked from [coleam00](https://github.com/coleam00/excalidraw-diagram-skill); icon approach from [awesome-copilot](https://github.com/github/awesome-copilot) (MIT) |
...
- **excalidraw-diagram** — `uv` plus a one-time `uv run playwright install chromium`. The Excalidraw render engine and fonts are vendored under `references/vendor/` (offline) — no CDN or Node needed at render time. Node + npm are needed only to re-vendor a newer Excalidraw version via `references/scripts/vendor.sh`. Optional cloud/architecture icons (AWS/Azure/GCP/K8s) use user-supplied `.excalidrawlib` files ingested by stdlib scripts — no extra tooling; sets aren't bundled (own licenses).

...
| [trilium-capture](./trilium-capture) | Write findings, clipped material and long-form documents into a self-hosted Trilium Notes instance over its **built-in** MCP — filed per project by label, one closed tag vocabulary, documents revised in place so Trilium's own revision history does the versioning | [Trilium Notes](https://github.com/TriliumNext/Trilium) MCP |
